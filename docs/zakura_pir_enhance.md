...
| `PrivateCandidate` | Withheld | Incoming memos and outgoing candidates |
...
been cryptographically ruled out. Completion is inferred from empty work queues;
...
- Both flags clear: apply the validated Ironwood data. Retire the dormant ordinary
  enhancement request only after every incoming and outgoing queue entry completes.
...
An explicit rescan requeues suspended candidates. Disabling private mode exposes
...
retired. Full transaction storage clears redundant private work. Rewinds prune
...
One migration creates four empty tables: incoming queue, outgoing queue, outgoing
candidate accounts, and transaction routing. It does not backfill existing
