...
impl Record {
    ...
    /// A mined or forked record must lie in the published window, and at the
    /// accepted anchor its block hash must agree with the tag.
    pub fn consistent_with(&self, manifest: &Manifest) -> bool {
        self.tag == 1
            || (self.height >= manifest.coverage_start
                && self.height <= manifest.anchor_height
                && (self.height != manifest.anchor_height
                    || (self.tag == 2) == (self.block == manifest.anchor_hash)))
    }
}
...
mod tests {
    ...
    #[test]
    fn records_at_the_anchor_must_agree_with_its_hash() {
        let m = manifest();
        for (tag, block, expected) in [
            (2, m.anchor_hash, Ok(Observation::Mined(20))),
            (2, [7; 32], Err(Error::Malformed)),
            (3, m.anchor_hash, Err(Error::Malformed)),
            (3, [7; 32], Ok(Observation::Forked)),
        ] {
            let row = single(tag, m.anchor_height, block);
