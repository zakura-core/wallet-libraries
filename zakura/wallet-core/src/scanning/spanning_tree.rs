//! A balanced binary tree over disjoint scan ranges.
//!
//! Ported from `zcash_client_backend::data_api::scanning::spanning_tree` with
//! its test suite intact. The merge rules this encodes — how two ranges that
//! overlap, contain or abut one another combine, and which of their priorities
//! survives — are subtle enough that reimplementing them from scratch would be
//! pure downside risk. The only changes are the removal of the upstream feature
//! gate and the module path.
...
/// A balanced binary tree over disjoint scan ranges.
///
/// Insertion preserves the existing partition points rather than rebalancing,
/// so the tree's shape reflects the order ranges arrived in; [`Self::into_vec`]
/// flattens it back to an ascending, merged sequence.
#[derive(Debug, Clone)]
pub enum SpanningTree {
    /// A single scan range.
    Leaf(ScanRange),
    /// Two subtrees covering adjacent, disjoint spans.
    Parent {
        /// The span covered by both children together.
        span: Range<BlockHeight>,
        /// The subtree covering the lower part of `span`.
        left: Box<SpanningTree>,
        /// The subtree covering the upper part of `span`.
...
impl SpanningTree {
    ...
    /// Inserts `to_insert`, merging it with whatever the tree already covers.
    ///
    /// Where the ranges overlap, the surviving priority is chosen by
    /// `dominance`: an already-`Scanned` range is preserved unless
    /// `force_rescans` is set, and otherwise the higher priority wins.
    ...
    /// Flattens the tree into ascending, non-empty, maximally-merged ranges.
...
/// Helpers for constructing scan ranges in tests.
pub mod testing {
    #![allow(missing_docs)]
    ...
    use crate::scanning::{ScanPriority, ScanRange};
...
mod tests {
    ...
    use crate::scanning::ScanPriority::*;
    use crate::scanning::{ScanPriority, ScanRange};
