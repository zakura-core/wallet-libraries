//! The scan-range queue model.
//!
//! Ported from `zcash_client_backend::data_api::scanning`, with its test suite
//! intact and one deliberate change: [`ScanPriority`] no longer carries a
//! `LatestPoolActivation` level.
//!
//! That level existed to surface the newest pool's balance before older history
//! had been backfilled, which is a problem only an ascending scan has. This
//! wallet recovers descending — tip to birthday — so the newest pool is reached
//! first by construction, and the priority that worked around its absence is
//! not needed. See `docs/wallet_core_rewrite.md`.
...
mod range_tests {
    ...
    #[test]
    fn a_range_reports_its_extent() {
        let r = ScanRange::from_parts((10u32.into())..(20u32.into()), ScanPriority::ChainTip);
        assert_eq!(r.len(), 10);
        assert!(!r.is_empty());
        assert_eq!(*r.block_range(), (10u32.into())..(20u32.into()));
        assert_eq!(r.priority(), ScanPriority::ChainTip);

        let empty = ScanRange::from_parts((10u32.into())..(10u32.into()), ScanPriority::Scanned);
        assert!(empty.is_empty());
        assert_eq!(empty.len(), 0);
    }

    #[test]
    fn a_range_renders_as_priority_and_bounds() {
        let r = ScanRange::from_parts((10u32.into())..(20u32.into()), ScanPriority::Verify);
        assert_eq!(r.to_string(), "Verify(10..20)");
    }

    #[test]
    #[should_panic(expected = "is invalid for ScanRange")]
    fn a_backwards_range_is_rejected() {
        ScanRange::from_parts((20u32.into())..(10u32.into()), ScanPriority::Historic);
    }

    #[test]
    fn priorities_order_from_ignored_to_verify() {
        // The ordering is the scheduling policy, so it is worth pinning.
        let ladder = [
